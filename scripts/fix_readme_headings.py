...
This script fixes the README.md file by:
1. Removing duplicate Authentication section
2. Fixing truncated headings
3. Arranging categories alphabetically
"""
...
def extract_category_sections(content):
    ...
    for anchor_id, title in sections:
        # Skip duplicate Authentication section
        if anchor_id == "authentication" and anchor_id in category_sections:
            continue
        ...
        if "![" in section_content and "brightgreen" not in section_content and "bri<a" in section_content:
            section_content = section_content.replace("bri<a", "brightgreen)</a>")
...
### {emoji} {category_name} ![0 APIs](https://img.shields.io/badge/0-APIs-brightgreen)
...
def fix_readme():
    ...
    try:
        ...
        # Sort categories alphabetically by name (without emoji)
        categories.sort(key=lambda x: x[0].split()[1] if len(x[0].split()) > 1 else x[0])
        ...
        print(f"Fixed README.md with {len(categories)} alphabetically sorted categories")
