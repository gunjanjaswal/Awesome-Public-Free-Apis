...
def update_readme_with_apis() -> None:
    ...
    try:
        ...
        # Regenerate ONLY the category tables between the markers, and keep everything
        # else (the header incl. the Ko-fi support badge, the nav, the Contributors
        # section, the trending sections and the footer) verbatim from the LIVE README.
        #
        # This deliberately does NOT read templates/README_template.md. That copy drifts
        # out of date, and using it as the source silently reverted manual header edits
        # (it dropped the Ko-fi badge) and would revert Contributors edits too. Sourcing
        # from the current README makes the script safe to run on every apis.json change.
        ...
        # Fail safe rather than destructive: if the markers are missing, refuse to
        # rewrite the file instead of blowing away everything after the header.
        if api_categories_start not in readme_content or api_categories_end not in readme_content:
            raise ValueError(
                "API categories markers not found in README.md; refusing to rewrite the "
                "file. Restore the BEGIN/END API CATEGORIES SECTION comments first."
            )
